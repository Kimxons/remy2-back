...
class DashboardNotificationsAsgiRegressionTests(TestCase):
    def setUp(self):
        self.client_api = APIClient()
        self.user = User.objects.create_user(
            username='notif_asgi_user',
            email='notif_asgi_user@example.com',
            password='StrongPass123!',
            role=Role.CLIENT,
            is_active=True,
        )
    ...
    def test_asgi_application_exposes_http_and_websocket_protocols(self):
        from config.asgi import application

        application_mapping = getattr(application, 'application_mapping', {})

        self.assertIn('http', application_mapping)
        self.assertIn('websocket', application_mapping)

    def test_dashboard_notifications_endpoint_returns_ok(self):
        Notification.objects.create(
            recipient=self.user,
            notification_type='JOB_CREATED',
        ...
        self.assertEqual(response.data[0]['type'], 'notification')
