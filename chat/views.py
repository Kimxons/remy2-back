...
class ChatThreadViewSet(viewsets.ModelViewSet):
    ...
    def get_permissions(self):
        if self.action in ['list', 'retrieve', 'create', 'sent_offers', 'mark_as_read']:
            return [AllowAny()]
    ...
    def check_permissions(self, request):
        ...
        if self.action not in ['list', 'retrieve', 'create', 'sent_offers', 'mark_as_read'] and not request.user.is_authenticated:
    ...
    def mark_as_read(self, request, pk=None, **kwargs): # Added **kwargs to catch thread_pk
        thread_id = kwargs.get('thread_pk') or pk or kwargs.get('pk')
        if not thread_id:
            raise ValidationError({'error': 'Thread id is required.'})
