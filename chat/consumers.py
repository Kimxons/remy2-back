...
class GlobalChatConsumer(AsyncWebsocketConsumer):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.legacy_guest_target: Optional[str] = None
    ...
    async def _initialize_connection(self):
        ...
        self.is_authenticated = bool(
            self.user and getattr(self.user, "is_authenticated", False)
        )
        ...
        self.legacy_guest_target = route_kwargs.get('legacy_guest_target')

        if not self.thread_id and not self.freelancer_id and self.legacy_guest_target:
            self.freelancer_id = await self._resolve_legacy_guest_target(
                self.legacy_guest_target
            )
        ...
        if self.legacy_guest_target and not self.freelancer_id:
            logger.warning(
                f"Legacy websocket target could not be resolved: {self.legacy_guest_target}"
            )
    ...
    def _resolve_legacy_guest_target(self, legacy_guest_target: str) -> Optional[str]:
        freelancer = User.objects.filter(
            Q(username__iexact=legacy_guest_target) |
            Q(email__iexact=legacy_guest_target)
        ).only('id').first()

        if freelancer:
            return str(freelancer.id)

        thread = ChatThread.objects.filter(
            Q(guest_session_key__iexact=legacy_guest_target) |
            Q(freelancer__username__iexact=legacy_guest_target)
        ).select_related('freelancer').only('freelancer_id').first()

        if thread:
            return str(thread.freelancer_id)
